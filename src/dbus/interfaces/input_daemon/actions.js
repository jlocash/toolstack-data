...
const actions = {
  ...
  attachVkbd: (domId) => dbusActions.sendMessage(service, path, iface, methods.ATTACH_VKBD, parseInt(domId)),
  ...
  authRemoteStatus: (autoStarted, status, id, username, recoveryKeyFile, ctxFlags) => dbusActions.sendMessage(service, path, iface, methods.AUTH_REMOTE_STATUS, autoStarted, parseInt(status), id, username, recoveryKeyFile, ctxFlags),
  ...
  authSetContextFlags: (user, title, flags) => dbusActions.sendMessage(service, path, iface, methods.AUTH_SET_CONTEXT_FLAGS, user, title, parseInt(flags)),
  authTitle: () => dbusActions.sendMessage(service, path, iface, methods.AUTH_TITLE),
  detachVkbd: (domId) => dbusActions.sendMessage(service, path, iface, methods.DETACH_VKBD, parseInt(domId)),
  ...
  lockTimeoutSet: (value) => dbusActions.sendMessage(service, path, iface, methods.LOCK_TIMEOUT_SET, parseInt(value)),
  ...
  setMouseSpeed: (value) => dbusActions.sendMessage(service, path, iface, methods.SET_MOUSE_SPEED, parseInt(value)),
  setSlot: (domId, slot) => dbusActions.sendMessage(service, path, iface, methods.SET_SLOT, parseInt(domId), parseInt(slot)),
  ...
  switchFocus: (domId, force) => dbusActions.sendMessage(service, path, iface, methods.SWITCH_FOCUS, parseInt(domId), force),
